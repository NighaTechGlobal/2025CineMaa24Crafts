...
interface ArtistSignupStepsProps {
    navigation: any;
    route?: {
        params?: {
            phone?: string;
...
const ArtistSignupSteps: React.FC<ArtistSignupStepsProps> = ({ navigation, route }) => {
    ...
    const handleFormComplete = async () => {
        ...
        if (!formData.firstName || !formData.email || !formData.gender || 
            !formData.department || !formData.state || !formData.city) {
            showDialog({
        ...
        try {
            ...
            // Store JWT (for JWT-only endpoints)
            if (response.token) {
    ...
    // Render Step 3 - Aadhar Verification
    const renderStep3 = () => (
        <ScrollView style={styles.stepContainer}>
...
export default ArtistSignupSteps;
